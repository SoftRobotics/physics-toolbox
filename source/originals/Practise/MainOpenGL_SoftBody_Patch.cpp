...
void init(float angle)
{
	...
	//Return value CreatePatch of btSoftbodyHelpers
	float s = 4;
	float h = 20;
	btSoftBody* softBody = btSoftBodyHelpers::CreatePatch(world->getWorldInfo(),
														btVector3(-s,h,-s),
														btVector3(s,h,-s),
														btVector3(-s,h,s),
	...
	softBody->setMass(100,100); //vertex Number = 100, static mass

	world->addSoftBody(softBody);

	//Add ellipsoid
	softBody = btSoftBodyHelpers::CreateEllipsoid(world->getWorldInfo(),btVector3(10,10,10),3.0); //radius = 3.0
	softBody->m_cfg.viterations = 50;
	softBody->m_cfg.piterations = 50;
	softBody->setTotalMass(3.0);//Mass of the softbody
