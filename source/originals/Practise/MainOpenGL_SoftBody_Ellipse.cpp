...
void renderSoftbody(btSoftBody* sb)
{
	//FACES
	...
	{
		for(int j=0;j<3;j++)// vertex = 3
		{
			glVertex3f(sb->m_faces[i].m_n[j]->m_x.x(),
					   sb->m_faces[i].m_n[j]->m_x.y(),
					   sb->m_faces[i].m_n[j]->m_x.y());
		}
	}
	glEnd();
	...
	glBegin(GL_TRIANGLES);
	for(int i=0;i<sb->m_links.size();i++)
	{
		for(int j=0;j<2;j++)// links = 2
		{
			glVertex3f(sb->m_links[i].m_n[j]->m_x.x(),
					   sb->m_links[i].m_n[j]->m_x.y(),
					   sb->m_links[i].m_n[j]->m_x.y());
		}
	}
	glEnd();
...
void init(float angle)
{
	...
	//Add ellipsoid
	btSoftBody* softBody = btSoftBodyHelpers::CreateEllipsoid(world->getWorldInfo(),
												  btVector3(10,10,10), //centre
												  btVector3(2,2,2),1000); //radius = 2,2,2 ; res = 1000
	softBody->m_cfg.viterations = 50;
	softBody->m_cfg.piterations = 50;
	softBody->m_cfg.kPR = 1000; //Put a pressure inside an elipse so it can get its shape back
	softBody->setTotalMass(3.0);//Mass of the softbody
	softBody->setMass(0,0); //Make one vertex to static
...
/*//#### Ray Tracing
	//Multiple object collision(Whichever object is within the range of camera gets blurred out)
	btVector3 direction = btVector3(0,0,5); //direction of Camera
	btVector3 position = btVector3(0,0,0); //position of Camera
	btCollisionWorld::AllHitsRayResultCallback rayCallback(position,direction);
	//Ray Tracing
	world->rayTest(position,direction,rayCallback);
	//if hit occurred,
	if(rayCallback.hasHit()){
		//Go through the array for all the collsion array
...
int _tmain() //used this because it can trasnlate to either main() or wmain()
{
	...
	//delete all softbodies to avoid leaking memory
	for(int i=0;i<world->getSoftBodyArray().size();i++){
