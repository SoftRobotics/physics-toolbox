...
#define CAMERA_TO_ARM_DISTANCE 20.0f
#define ARM_ELBOW_Y_POSITION 11.0f
#define ARM_END_EFFECTOR_Y_POSITION 21.0f
...
m_pickConstraint(0),
m_shootBoxShape(0),
m_cameraDistance(CAMERA_TO_ARM_DISTANCE),
m_debugMode(0),
m_ele(20.f),
m_azi(0.f),
m_cameraPosition(0.f, ARM_END_EFFECTOR_Y_POSITION, 0.f),
m_cameraTargetPosition(0.f, ARM_ELBOW_Y_POSITION, 0.f),
m_mouseOldX(0),
m_mouseOldY(0),
m_mouseButtons(0),
m_modifierKeys(0),
...
void DemoApplication::keyboardCallback(unsigned char key, int x, int y) {
    ...
    switch (key) {
        ...
        case ' ':
        {
            static bool applyingForce = false;
            if(applyingForce){
                getDynamicsWorld()->setGravity(btVector3(0,0,0));
                applyingForce = false;
            }
            else{
                getDynamicsWorld()->setGravity(btVector3(50,0,0));
                applyingForce = true;
            }
            break;
        }
...
void DemoApplication::renderscene(int pass) {
    ...
    for (int i = 0; i < numObjects; i++) {
        ...
        if (!(getDebugMode() & btIDebugDraw::DBG_DrawWireframe)) {
            switch (pass) {
                case 0: m_shapeDrawer->drawOpenGL(m, colObj->getCollisionShape(), wireColor, getDebugMode(), aabbMin, aabbMax);
                    break;
                case 1: m_shapeDrawer->drawShadow(m, m_sundirection*rot, colObj->getCollisionShape(), aabbMin, aabbMax);
                    break;
                case 2: m_shapeDrawer->drawOpenGL(m, colObj->getCollisionShape(), wireColor * btScalar(0.3), 0, aabbMin, aabbMax);
                    break;
            }
        }
    }
